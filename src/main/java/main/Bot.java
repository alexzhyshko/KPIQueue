			...
			case 1:
				if (message.equals("Add queue")) {
					...
					if (names.contains(message)) {
						...
						setButtons(Arrays.asList("Sign in", "Show queue", "Remove me", "I passed", "Delete queue", "Back"), response);
						user.userstate = 2;
					} else {
					...
					if (twoWeek) {
						if (weekNum % 2 == 0 && evenweek) {
							if (LocalDate.now().getDayOfWeek().compareTo(queue.day.minus(2)) >= 0
								...
								if (LocalTime.now().isAfter(LocalTime.of(18, 0, 0))&&LocalTime.now().isBefore(LocalTime.of(queue.hour+1, queue.minute+30, 0))) {
									if (stack.push(user)) {
					...
					for(Entry e : queues.entrySet()) {
						if(((Queue)(e.getKey())).name.equals(session.get("queuename"))) {
							...
							if(creators.get(tmp).equals(userid)) {
								...
								List<String> buttons = queues.keySet().stream().map(o -> o.name).collect(Collectors.toList());
								buttons.add("Add queue");
								setButtons(buttons, response);
								user.userstate = 1;
								break;
